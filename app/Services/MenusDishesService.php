...
use App\Interfaces\MenusDishesInterFace;
use App\Models\Day;
...
class MenusDishesService implements MenusDishesInterFace
{
    use Menus;

    public static function get_products_bju($products_id, $dishes_id, $field){
        $koef = self::getKoefBJU()[$field];
    ...
    public static function get_bju_dish($menus_dishes_id, $field){
        ...
        foreach($dishes_products as $d_product){
            $value = self::get_products_bju($d_product->products_id, $m_dishes->dishes_id, $field) * ($d_product->net_weight/100) *($m_dishes->yield / $dishes->yield);/**/
            $total = $total + $value;
        }
        return round($total, 1);
    }

    public static function get_kkal($products_id, $menus_dishes_id){
        $m_dishes = MenuDishes::where('id', $menus_dishes_id)->first();
    ...
    public static function get_kkal_dish($menus_dishes_id){//per dish
        $m_dishes = MenuDishes::where('id', $menus_dishes_id)->first();
        $dishes = Dishes::where('id', $m_dishes->dishes_id)->first();
        $total = 0;
        $dishes_products = DishesProducts::where('dishes_id', $m_dishes->dishes_id)->get();
        foreach($dishes_products as $d_product){
            $kkal = self::get_kkal($d_product->products_id, $m_dishes->id) * ($d_product->net_weight/100) *($m_dishes->yield / $dishes->yield);/**/
            $total = $total + $kkal;
        }
        return round($total, 1);
    }
    ...
    public static function get_vitamin($menus_dishes_id, $yield, $field){
        $m_dishes = MenuDishes::where('id', $menus_dishes_id)->first();
        $dishes = Dishes::where('id', $m_dishes->dishes_id)->first();
        $dishes_products = DishesProducts::where('dishes_id', $m_dishes->dishes_id)->get();
        $sum = 0;
        if ($dishes->culinary_processing_id != 3){
            $uvarka = self::getUvarka()[$field];
        }else{
            $uvarka = 1;
        }
